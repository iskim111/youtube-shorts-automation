...
#### 설정·트렌드·캐릭터·참조 (2026-07 추가)

| Method | Path | 설명 | Role |
|---|---|---|---|
| GET/PUT | `/api/v1/settings/keys` | API 키 조회·저장 (마스킹) | admin |
| GET | `/api/v1/trending/shorts` | KR 인기 Shorts TOP N | operator+ |
| POST | `/api/v1/trending/shorts/{video_id}/create-production` | 트렌드 선택 → 시나리오·Job | operator+ |
| GET | `/api/v1/characters` | AI 캐릭터 목록 | operator+ |
| POST | `/api/v1/characters` | 캐릭터 추가 | admin |
| PUT | `/api/v1/characters/{id}` | 캐릭터 수정·비활성화 | admin |
| GET | `/api/v1/characters/presets` | 시리즈 프리셋 목록 | operator+ |
| POST | `/api/v1/characters/series/episode` | 2인 대화 에피소드 Job 생성 | operator+ |
| POST | `/api/v1/references/analyze` | 참조 URL 메타 분석 | operator+ |
| POST | `/api/v1/references/create-job` | 참조 기반 Job 생성 | operator+ |
| POST | `/api/v1/jobs/{id}/chat` | AI 채팅 대본 수정 | editor+ |
| POST | `/api/v1/jobs/{id}/run` | 파이프라인 실행·재렌더 | operator+ |
...
| 화면 | 경로 | 핵심 컴포넌트 |
|---|---|---|
| **Topic Lab** | `/topics` | TOP 100, 시리즈 에피소드, 참조 URL, AI 주제 변형, Job 워크스페이스(미리보기·채팅) |
| **Job Detail** | `/jobs/[id]` | 스테이지 타임라인, 스크립트, 파이프라인 Run, AI 채팅 수정, 재렌더 |
| **Settings** | `/settings` | API 키, AI 캐릭터(주인공), OAuth, 채널, 준비 체크리스트 |
| ~~Overview~~ | `/overview` | (레거시) 홈 — 기본 진입은 `/topics` |
| ~~Rights Center~~ | `/rights` | (레거시) 검수 큐 — 필요 시 재활성 |
| ~~Upload Calendar~~ | `/calendar` | (레거시) 예약 슬롯 |
| ~~Analytics~~ | `/analytics` | (레거시) KPI |
| ~~Audit~~ | `/audit` | (레거시) 감사 로그 |

**2026-07 운영 UI 축소**: 사이드바는 **Topic Lab** + **Settings** 2탭만 노출. 생성·검수·채팅은 Topic Lab 단일 워크스페이스에서 완료.
...
# External APIs
OPENAI_API_KEY=...
YOUTUBE_API_KEY=...          # Data API — TOP 100 수집
ELEVENLABS_API_KEY=...       # 캐릭터 TTS
HEYGEN_API_KEY=...           # AI 캐릭터 영상
GCP_PROJECT_ID=...
YOUTUBE_CLIENT_ID=...
...
# Production mode
VIDEO_MODE=ai_character      # ai_character | stock_broll
TRENDING_CACHE_TTL_HOURS=6
...
| 음성 정책 | 일반 합성음만; **허구 AI 캐릭터**는 ElevenLabs Voice ID 허용. 실존 유명인 보이스 클론·립싱크 금지 |
| 업로드 빈도 | 1채널, 일 3~5개 |
| 영상 길이 | 45초 이하 |
| 목표 채널 수 | 1개 (파일럿) |
| 예산 | 파일럿 구간 ($100~350/월) |
...
10. ~~**AI 쇼츠 제작 고도화 (2026-07)**~~ ✅ Topic Lab 통합, TOP 100, 캐릭터, HeyGen/ElevenLabs, Settings API 키
...
| API 키 | Settings → API 키 저장 | OpenAI, YouTube Data, ElevenLabs, HeyGen |
| AI 캐릭터 | Settings → 주인공 추가 | Avatar ID + Voice ID |
| 실제 업로드 | `.env` → `PILOT_DRY_RUN_UPLOAD=false` | OAuth + FFmpeg 필수 |

---

## 20. AI 쇼츠 제작 고도화 (2026-07 구현)

### 20.1 설계 방향

| 원칙 | 내용 |
|---|---|
| **실존 인물 미사용** | 유명인(예: 호날두) 참조 시 **포맷·주제만** 차용. 얼굴·원본 영상·음성 립싱크 재사용 금지 |
| **AI 캐릭터 우선** | HeyGen 아바타 + ElevenLabs TTS로 **허구 주인공** 시리즈 운영 |
| **단일 워크스페이스** | Topic Lab에서 주제 선택 → Job → Run → 미리보기 → 채팅 수정 → 재렌더 |
| **API 키 UI 관리** | Settings에서 입력 → DB(`app_settings`) 저장, `.env` fallback |

### 20.2 Topic Lab 워크플로

```text
[A] 인기 Shorts TOP 100
    YouTube Data API (search.list + videos.list, KR, short, viewCount)
    → 항목 선택 → OpenAI 시나리오(동일 주제·오리지널) → Job

[B] 시리즈 캐릭터 에피소드
    프리셋 (grandma_youth_en 등) + 등록된 캐릭터(role 매칭)
    → OpenAI 2인 대화 시나리오 → Job (format=dialogue)

[C] 참조 Shorts URL
    oEmbed (제목·썸네일·채널) → 대본 시드 → Job
    ※ 영상/음성 다운로드 없음 (분석용 yt-dlp는 후속 Phase)

[D] AI 주제 변형 (접이식)
    OpenAI + recent_hooks 회피, temperature↑
    고정 풀(trending_topics_ko) 의존 축소
```
...
| 필드 | 설명 |
|---|---|
| code | CHR-001, CHR-GRANDMA 등 |
| name | 표시 이름 |
| role | grandmother, youth, host, guest — 시리즈 프리셋 매칭 |
| heygen_avatar_id | HeyGen 아바타 |
| elevenlabs_voice_id | ElevenLabs 보이스 |
| speech_style | 말투 규칙 (프롬프트 힌트) |
| is_active | 비활성화 지원 |
...
| ID | 설명 | roles |
|---|---|---|
| `grandma_youth_en` | 할머니+외국 청년, 한국어 대화 중 영어 문장·설명 | grandmother, youth |
| `interview_ai` | AI 인터뷰 포맷 (허구 진행자·게스트) | host, guest |

Settings UI에서 **주인공 추가·수정·비활성화** 지원.

### 20.4 영상 파이프라인 분기

```text
POST /jobs/{id}/run
  → script (기존 또는 OpenAI 시드)
  → tts (stub | ElevenLabs — 장면별)
  → asset (Pexels | placeholder)
  → render:
       if VIDEO_MODE=ai_character AND heygen_configured AND format=dialogue:
           ai_video_pipeline (HeyGen per scene → FFmpeg concat)
       else:
           render_engine (FFmpeg template + stock)
  → subtitle SRT → metadata → QA_PENDING
```
...
```json
{
  "format": "dialogue",
  "hook": "...",
  "character_codes": ["CHR-GRANDMA", "CHR-YOUTH"],
  "scenes": [
    {
      "seq": 1,
      "character_code": "CHR-GRANDMA",
      "narration": "안녕, 오늘 뭐 배워볼까?",
      "language": "ko",
      "duration_sec": 6
    }
  ],
...
### 20.5 채팅 수정 (JobReviewChat)

| 단계 | 동작 |
|---|---|
| GET `/jobs/{id}/preview` | 영상 URL, script, reference 메타 |
| POST `/jobs/{id}/chat` | OpenAI 또는 규칙 기반 대본 수정 |
| 재Run | 수정 script → TTS/렌더 재실행 |

채팅 예: `훅: 새 문장`, `장면 2: 나레이션`, `분위기를 더 유머러스하게`

### 20.6 외부 API 역할 (최적 조합)

| API | 용도 | 필수 |
|---|---|---|
| **OpenAI** | 시나리오, 주제 변형, 채팅 수정, (후속) Whisper/Vision | ✅ |
| **YouTube Data API** | KR 인기 Shorts TOP 100 | ✅ |
| **ElevenLabs** | 캐릭터별 한국어·영어 TTS | ✅ (AI 캐릭터) |
| **HeyGen** | AI 아바타 말하기 영상 | ✅ (AI 캐릭터) |
| **Pexels/Pixabay** | B-roll fallback | 선택 |
| **YouTube OAuth** | 실제 업로드 | 업로드 시 |

**의도적 미사용**: 실사 얼굴 립싱크(Sync 등) + 유명인 참조 영상 재업로드 — 초상권·저작권·유튜브 정책 리스크.

### 20.7 신규/변경 모듈

| 경로 | 역할 |
|---|---|
| `services/settings_store.py` | DB + .env 병합 설정 |
| `services/trending_shorts_service.py` | TOP 100 수집·캐시 |
| `services/scenario_generator.py` | 트렌드·2인 대화 시나리오 |
| `services/production_service.py` | 트렌드/시리즈 → Topic+Job |
| `services/reference_analyzer_service.py` | 참조 URL oEmbed 분석 |
| `services/tts_service.py` | ElevenLabs |
| `services/heygen_service.py` | HeyGen 영상 생성 |
| `services/ai_video_pipeline.py` | 장면 합성 |
| `services/character_service.py` | 캐릭터 CRUD·시드 |
| `models/app_setting.py` | API 키 저장 |
| `models/character.py` | 주인공 |

### 20.8 후속 Phase (미구현)

| 항목 | 설명 |
|---|---|
| yt-dlp + Whisper | 참조 영상 **구조·대사 흐름** 분석 (복사 금지) |
| 채팅 → 부분 재생성 | 변경 장면만 TTS/HeyGen 재호출 |
| 시리즈 캐릭터 UI 선택 | 프리셋 외 임의 2인 조합 |
| 임베딩 유사도 | 주제 중복 하드 필터 |
| TOP 100 OpenAI 클러스터링 | 제목 요약·중복 제거 |

---

## 21. 확정 의사결정 (2026-07-05)

| 항목 | 확정값 |
|---|---|
| 운영 UI | **Topic Lab + Settings** 2탭 |
| 영상 기본 모드 | **ai_character** (HeyGen), 실패 시 stock_broll FFmpeg |
| 트렌드 소스 | YouTube Data API TOP 100 (고정 풀 보조) |
| 참조 URL | oEmbed 메타만; 실사 립싱크·원본 재사용 금지 |
| 주인공 | DB `characters` + UI 추가·비활성화 |
| API 키 | Settings UI → `app_settings` 테이블 |

---

*본 설계안은 기획안의 정책·아키텍처 방향을 구현 가능한 모듈·스키마·API·화면 단위로 구체화한 문서이다. 2026-07 AI 캐릭터·TOP 100·Topic Lab 통합 워크플로를 반영하였다. 구현 중 세부 수치(유사도 임계치, 스코어 가중치)는 파일럿 데이터로 보정한다.*
